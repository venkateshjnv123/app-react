...
export default function Department({navigation}){
    ...
    const  getUsers = () => {
      firebase.database().ref('Data').child('Government').child('Departments').on('value', function (snapshot) {
 ...
 {/* <TouchableOpacity onPress={()=>navigation.navigate('Hoscard', item)}>
   <View style={styles.card3}>
     <Text>{item.Name}</Text>
             ...
             <TouchableOpacity style={styles.card} onPress={()=> navigation.navigate('DepartmentCard', item)}>
